...
- **宁缺毋滥**：用例数量取决于需求的复杂度，不设最低条数。每条用例必须测试一个独立的、有意义的测试点。需求简单（如单字段输入框）可能只有 5-10 条，需求复杂才接近上限。禁止用参数排列组合凑数
...
2. 必须覆盖以下类型各至少 1 条：正向功能、边界值、异常场景。上限不超过 66 条。66 是上限不是目标 — 需求简单就少生成，每条必须有独立测试价值，禁止凑数。禁止用组合爆炸（换浏览器/换 OS/换分辨率等）填充数量
3. **用例标题只描述用例场景，禁止带类型标签**（如"【正向】""【异常】""【边界】"等），类型信息由 type 字段独立承载
4. **优先级规划（强制，生成用例前必须执行）：**

   **Step 1 — 确定总数：** 根据需求复杂度估算本次要生成的用例总数 N，取最接近的对照表档位（10/15/20/30/40/50）。

   **Step 2 — 查表得分布：** 对照下表确定各优先级目标条数（±1 条容差）。

   **Step 3 — 逐条生成：** 按目标分布生成用例，生成过程中持续计数，确保不偏离。

   **Step 4 — 输出前核对：** 统计实际各优先级数量，与目标对比。不匹配则调整优先级后重新核对。
   ...
   **条数对照表（±1 条容差）：**
   ...
   严禁全部标 P0、全部标 P3、或跳过任一优先级。P4 在总用例数 ≤20 时可省略。
