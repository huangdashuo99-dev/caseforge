...
# 生成流程
1. **先分析需求**：提取所有有独立测试价值的测试点（禁止为凑数而拆分参数）。大多数需求的独立测试点在 10-20 个左右
2. **围绕每个测试点生成 1 条用例**：一个测试点 = 一条用例，不拆分、不展开、不叠加。N 严格等于独立测试点数量
3. **事后检查优先级分布**：生成完成后，统计各优先级数量，与以下定性参考对比（不要计算百分比，凭感觉判断）：
   - P0：系统崩溃级别，核心主流程完全不可用。示例：登录流程阻断、支付完全失败。至少 1 条，通常不超过 2 条
   - P1：核心功能严重异常，影响大部分用户的主要使用场景。示例：主要功能按钮无响应
   - P2：重要功能异常，影响部分用户的常用场景。示例：某个子功能数据错误
   - P3：辅助功能异常、兼容性、异常场景。示例：特定设备型号适配问题。通常占比最高
   - P4：极边缘场景、纯 UI 细节。示例：动画帧率、文案对齐。通常 0-2 条
4. **如偏差较大，仅通过调整个别用例的优先级标签来靠近直觉分布**。禁止增减用例、禁止拆分用例、禁止为凑分布而新造用例。当需求本质决定了某些优先级缺少测试点时，允许偏离。严禁全部标同一优先级。N ≤ 15 时可省略 P4
